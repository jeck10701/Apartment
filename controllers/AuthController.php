    ...
    if ($action === 'upload_avatar') {
        ...
        try {
            ...
            if (!empty($_POST['cropped_image_data'])) {
                ...
                if (preg_match('/^data:image\/([a-zA-Z0-9_\-\+]+);base64,(.+)$/is', $rawCrop, $matches)) {
                ...
                if ($decoded === false || strlen($decoded) === 0) {
                    throw new Exception('Invalid cropped image data. Please try choosing the photo again.');
                }

                if (strlen($decoded) > 15 * 1024 * 1024) {
                    throw new Exception('Cropped image exceeds the 15 MB limit.');
                }
                ...
                if (file_put_contents($destination, $decoded) === false) {
                    throw new Exception('Unable to save cropped image to server directory.');
                }
            } elseif (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
                $file = $_FILES['avatar'];
                if ($file['size'] > 15 * 1024 * 1024) {
                    throw new Exception('Profile picture must be 15 MB or smaller.');
                }

                $imageInfo = @getimagesize($file['tmp_name']);
                if ($imageInfo === false) {
                    throw new Exception('Please upload a valid image file (JPG, JPEG, PNG, WEBP).');
                }
                ...
                if (!isset($allowedMime[$mime])) {
                    throw new Exception('Only JPG, JPEG, PNG, and WEBP profile pictures are allowed.');
                }
